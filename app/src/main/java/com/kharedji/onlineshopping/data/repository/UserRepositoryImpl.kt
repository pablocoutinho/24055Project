package com.kharedji.onlineshopping.data.repository
...
class UserRepositoryImpl() : UserRepository {
     ...
     */
    override suspend fun signUpUser(userData: UsersData, password: String) = flow {
        emit(State.loading())
        auth.createUserWithEmailAndPassword(userData.email, password).await().run {
            if (this.user!=null){
    ...
    fun saveToDatabase(userData: UsersData){
        val firebaseDatabase=FirebaseDatabase.getInstance().reference.child("users")
        firebaseDatabase.child(userData.id).setValue(userData)
    }
